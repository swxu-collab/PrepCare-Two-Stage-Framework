#!/usr/bin/env python3
"""Plot Stage-2 comparator trade-off: burden vs event precision."""
...
OUT_STEM = "stage2_tradeoff_precision_vs_burden"
...
def main() -> None:
    ...
    for method, g in df.groupby("method"):
        ...
        for _, row in g.iterrows():
            ax.annotate(
                f"ρ={row['rho']:.2f}",
                (row["total_alerts_monitor"], row["top3_event_precision"]),
    ...
    ax.set_ylabel("Top-3 event precision")
    ax.set_title("Stage-2 Alerting Trade-off: Burden vs Precision")
